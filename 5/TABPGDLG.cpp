...
void __fastcall TPagesDlg::N5Click(TObject *Sender) {
	if (MessageDlg("Выйти?", mtWarning, TMsgDlgButtons() << mbOK, 0) == IDOK) {
		PagesDlg->Close();
	}
}
// ---------------------------------------------------------------------------
...
void __fastcall TPagesDlg::DeleteButtonClick(TObject *Sender) {
	if (MessageDlg("Вы хотитe удалить поле?", mtWarning,
		TMsgDlgButtons() << mbOK, 0) == IDOK) {
...
void __fastcall TPagesDlg::ClearButtonClick(TObject *Sender) {
	if (MessageDlg("Вы хотите очистить поля?", mtWarning,
		TMsgDlgButtons() << mbOK, 0) == IDOK) {
		AddNameEdit->Clear();
		AddMassEdit->Clear();
		AddSizeEdit->Clear();
...
void __fastcall TPagesDlg::AddButtonClick(TObject *Sender) {
	if (MessageDlg("Вы хотитe добавить поле?", mtWarning,
		TMsgDlgButtons() << mbOK, 0) == IDOK) {
		StringGrid1->Cells[0][4] = AddNameEdit->Text;
		StringGrid1->Cells[1][4] = AddMassEdit->Text;
		StringGrid1->Cells[2][4] = AddSizeEdit->Text;
		StringGrid1->Cells[3][4] = AddMatEdit->Text;
		ComboBox1->Items->Add(StringGrid1->Cells[0][4]);
		AddNameEdit->Clear();
		AddMassEdit->Clear();
		AddSizeEdit->Clear();
		AddMatEdit->Clear();
...
void __fastcall TPagesDlg::N6Click(TObject *Sender) {
	if (MessageDlg("Вы очистить таблицу?", mtWarning, TMsgDlgButtons() << mbOK,
		0) == IDOK) {
		for (int i = 1; i < StringGrid1->ColCount; i++)
			for (int j = 0; j < StringGrid1->RowCount; j++)
